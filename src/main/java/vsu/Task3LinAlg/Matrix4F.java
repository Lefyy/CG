...
public class Matrix4F implements Matrix {
    ...
    public Vector4F getRow(int index) {
        if (index < 0 || index > matrix.length) {
            ...
            return new Vector4F(matrix[index][0], matrix[index][1], matrix[index][2], matrix[index][3]);
        }
    }

    @Override
    public Vector4F getCol(int index) {
        if (index < 0 || index > matrix[0].length) {
            throw new IllegalArgumentException("Такого индекса нету у меня");
        } else {
            return new Vector4F(matrix[0][index], matrix[1][index], matrix[2][index], matrix[3][index]);
        }
    }
    ...
    public void setRow(int index, float[] values) {
        if (index < 0 || index > matrix.length || values.length != matrix.length) {
            throw new IllegalArgumentException("Что что ты хочешь сделать с матрицей 4на4");
        } else {
            for (int i = 0; i < matrix.length; i++) {
    ...
    public Matrix4F getInverse() {
        ...
        if (det != 0) {
            Matrix4F complements = getComplement();
    ...
    public Matrix4F getComplement() {
        ...
        return new Matrix4F(complement);
    }

    @Override
    public Vector4F gaussSolution(Vector b) {
        if (b instanceof Vector4F) {
    ...
    public boolean compareTo(Matrix m) {
        if (m instanceof Matrix4F) {
            ...
            for (int i = 0; i < matrix.length; i++) {
                for (int j = 0; j < matrix.length; j++) {
                    if (Math.abs(m.get(i, j) - matrix[i][j]) > 0.1e-9) {
                        similar = false;
                        return similar;
                    }
                }
